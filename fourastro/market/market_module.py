...
def add_structural_direction(historical_data):
    ...
    # Initialize structural_direction with NaN. The first element will remain NaN
    # as there's no previous data point to compare against.
    structural_direction = np.full(num_rows, np.nan)

    # Iterate from the second data point (index 1) to compare with the previous day
    for i in range(1, num_rows):
        ...
        # Condition 1: Ascending (New High Structure)
        if h_t > h_t_minus_1 and l_t >= l_t_minus_1:
            structural_direction[i] = 1
        # Condition 2: Descending (New Low Structure)
        elif l_t < l_t_minus_1 and h_t <= h_t_minus_1:
            structural_direction[i] = -1
        # Condition 3: Neutral (Continuation/Indecisive)
        else:
            # S_d(t) = S_d(t-1)
